# Locally modified from the Apache-2.0 upstream source for offline processor
# loading and explicit vision-error propagation.
...
class Qwen3VLEmbedder():
    ...
    def _preprocess_inputs(self, conversations: List[List[Dict]]) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            # Let the caller isolate the failing record.  The upstream fallback
            # replaces an arbitrary-size batch with one NULL sample, which can
            # silently misalign row-level predictions.
