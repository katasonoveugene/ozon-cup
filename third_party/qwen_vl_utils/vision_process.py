# Locally modified from the Apache-2.0 upstream source to make the unused HTTP
# dependency optional during local-file-only inference.
...
def fetch_image(ele: Dict[str, Union[str, Image.Image]], image_patch_size: int = 14) -> Image.Image:
    ...
    if isinstance(image, Image.Image):
        ...
        if requests is None:
            raise RuntimeError("HTTP image loading requires the optional requests package")
