...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-i", "--test_data_path", "--test-data-path", dest="test_data_path", type=Path, required=True
    )
    parser.add_argument(
        "-o", "--output_path", "--output-path", dest="output_path", type=Path, required=True
    )
...
def validate_input(frame: pd.DataFrame) -> None:
    ...
    if frame.empty or frame["id"].duplicated().any():
        raise ValueError("input must be non-empty and IDs must be unique")
...
def validate_output(frame: pd.DataFrame, output: pd.DataFrame) -> None:
    ...
    for value in output["result"].astype(str):
        ...
        if not 50 <= len(comment) <= 300 or verdict not in {"бан", "не бан"}:
            raise RuntimeError("output comment or verdict is invalid")
...
def main() -> None:
    ...
    config = json.loads((root / "artifacts" / "runtime.json").read_text(encoding="utf-8"))
    model = joblib.load(root / "artifacts" / "validated_blend.joblib")
    embeddings = extract_embeddings(
        ...
        max_pixels=int(config["embedding_max_pixels"]),
    )
    _, predictions, _ = model.predict(frame, embeddings)
    comments = generate_comments(
        frame,
        predictions,
        model_path=shared_models / "Qwen" / "Qwen3.5-4B",
        batch_size=int(config["comment_batch_size"]),
        max_new_tokens=int(config["comment_max_new_tokens"]),
    )
    output = pd.DataFrame(
